...
nofafsa_repop_results <- 
  nofafsa_df_repop %>% 
  #filter(fafsa_completed==0) %>% 
  group_by(new_groups, new_test_or_control) %>% 
  summarise(
      InPopulation = n(), 
      #`Funding Tasks` = sum(funding_task),
      #`Funding %` = sum(funding_task)/n(),
 ...
 gt() %>%
   ...
   tab_header(
    title = md("21EW4 No FAFSA Dialing & Tasks: Test vs. Control")) %>% 
    tab_source_note(
    source_note = c("Data Source:  Unify Opportunity, SSR - retrieved on 3/3/21"))
  #%>%
     #data_color(columns = vars(`Melt Rate`), colors = "lightgreen") %>%
    #tab_options(
      # column_labels.font.size = 13,
...
```{r}
# Overall results save
nofafsa_repop_results %>% gtsave(filename = "nofafsa_21EW4_overall_20210303_repop.png", path = here::here("imgs"))
```
