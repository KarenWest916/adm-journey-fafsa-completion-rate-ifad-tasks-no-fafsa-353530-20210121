...
nofafsa_repop_results <- 
 ...
 gt() %>%
   ...
   tab_header(
    title = md("21EW4 No FAFSA Dialing & Tasks: Test vs. Control")) %>% 
    tab_source_note(
    source_note = c("Data Source:  Unify Opportunity, SSR - retrieved on 3/19/21"))
  #%>%
     #data_color(columns = vars(`Melt Rate`), colors = "lightgreen") %>%
    #tab_options(
      # column_labels.font.size = 13,
...
```{r}
# Overall results save
nofafsa_repop_results %>% gtsave(filename = "nofafsa_21EW4_overall_20210319_repop.png", path = here::here("imgs"))
```
...
```{r}
# Overall results save
nofafsa_repop_results_melt %>% gtsave(filename = "nofafsa_21EW4_melt_20210317_repop.png", path = here::here("imgs"))
```
